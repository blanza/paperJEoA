...
last_before_proj_aux = 2015 - proj_length_aux
...
if (check == "N") {
  ...
  years = seq(projStartYear, last_before_proj_aux)
  years_forecast = seq(tail(years, n = 1) + 1, tail(years, n = 1) + proj_length_aux)
  years_all = seq(years[1], tail(years_forecast, n = 1))
}
...
if (check == "Y") {
  ...
  LFPRMale_interpolation_check = subset(LFPRMale_interpolation, rownames(LFPRMale_interpolation) %in%
                                          years)
  MaleModel_check = leecarter(LFPRMale_interpolation_check, proj_length_aux)
  FLFPRMale_check <- matrix(nrow = length(MaleModel_check$kt_forecast), ncol = length(ages))
  FLFPRMale_hi_check <- matrix(nrow = length(MaleModel_check$kt_hi_forecast), ncol = length(ages))
  FLFPRMale_lo_check <- matrix(nrow = length(MaleModel_check$kt_lo_forecast), ncol = length(ages))
  
  for (i in 1:length(MaleModel_check$kt_forecast)) {
    FLFPRMale_check[i, ] <- exp((MaleModel_check$bx * MaleModel_check$kt_forecast[i]) +
                                  MaleModel_check$ax)
    FLFPRMale_hi_check[i, ] <- exp((MaleModel_check$bx * MaleModel_check$kt_hi_forecast[i]) +
                                  MaleModel_check$ax)
    FLFPRMale_lo_check[i, ] <- exp((MaleModel_check$bx * MaleModel_check$kt_lo_forecast[i]) +
                                  MaleModel_check$ax)
  }
  
  LFPRFemale_interpolation_check = subset(LFPRFemale_interpolation, rownames(LFPRFemale_interpolation) %in%
                                            years)
  FemaleModel_check = leecarter(LFPRFemale_interpolation_check, proj_length_aux)
  FLFPRFemale_check <- matrix(nrow = length(FemaleModel_check$kt_forecast), ncol = length(ages))
  FLFPRFemale_hi_check <- matrix(nrow = length(FemaleModel_check$kt_hi_forecast), ncol = length(ages))
  FLFPRFemale_lo_check <- matrix(nrow = length(FemaleModel_check$kt_lo_forecast), ncol = length(ages))
  for (i in 1:length(FemaleModel_check$kt_forecast)) {
    ...
    FLFPRFemale_hi_check[i, ] <- exp((FemaleModel_check$bx * FemaleModel_check$kt_hi_forecast[i]) +
                                    FemaleModel_check$ax)
    FLFPRFemale_lo_check[i, ] <- exp((FemaleModel_check$bx * FemaleModel_check$kt_lo_forecast[i]) +
                                    FemaleModel_check$ax)
  }
  ...
  FLFPRMale_hi_check <- t(FLFPRMale_hi_check)
  FLFPRMale_hi_check <- as.data.frame(FLFPRMale_hi_check)
  rownames(FLFPRMale_hi_check) = ages
  colnames(FLFPRMale_hi_check) = years_forecast
  FLFPRMale_hi_check = melt(t(FLFPRMale_hi_check))
  colnames(FLFPRMale_hi_check) = c("Year", "Age", "Ratio")
  
  FLFPRMale_lo_check <- t(FLFPRMale_lo_check)
  FLFPRMale_lo_check <- as.data.frame(FLFPRMale_lo_check)
  rownames(FLFPRMale_lo_check) = ages
  colnames(FLFPRMale_lo_check) = years_forecast
  FLFPRMale_lo_check = melt(t(FLFPRMale_lo_check))
  colnames(FLFPRMale_lo_check) = c("Year", "Age", "Ratio")
  
  FLFPRMale_check$Ratio_hi <- FLFPRMale_hi_check$Ratio
  FLFPRMale_check$Ratio_lo <- FLFPRMale_lo_check$Ratio
  ...
  FLFPRFemale_hi_check <- t(FLFPRFemale_hi_check)
  FLFPRFemale_hi_check <- as.data.frame(FLFPRFemale_hi_check)
  rownames(FLFPRFemale_hi_check) = ages
  colnames(FLFPRFemale_hi_check) = years_forecast
  FLFPRFemale_hi_check = melt(t(FLFPRFemale_hi_check))
  colnames(FLFPRFemale_hi_check) = c("Year", "Age", "Ratio")
  
  FLFPRFemale_lo_check <- t(FLFPRFemale_lo_check)
  FLFPRFemale_lo_check <- as.data.frame(FLFPRFemale_lo_check)
  rownames(FLFPRFemale_lo_check) = ages
  colnames(FLFPRFemale_lo_check) = years_forecast
  FLFPRFemale_lo_check = melt(t(FLFPRFemale_lo_check))
  colnames(FLFPRFemale_lo_check) = c("Year", "Age", "Ratio")
  
  FLFPRFemale_check$Ratio_hi <- FLFPRFemale_hi_check$Ratio
  FLFPRFemale_check$Ratio_lo <- FLFPRFemale_lo_check$Ratio
  
  cols <- c(Observed = "#15B7B9", Projected = "#F57170")
  
  MalePlot_check = ggplot() + 
    geom_ribbon(data = subset(FLFPRMale_check, Year %in% 2015 & Age >= 20), aes(x = Age, ymin = Ratio_lo, ymax = Ratio_hi), fill = "grey70", alpha = 0.25) + 
    geom_line(data = subset(FLFPRMale_check, Year %in% 2015 & Age >= 20), aes(x = Age, y = Ratio, colour = "2015 - Projected")) +
    geom_line(data = subset(LFPRMale_interpolation_check, Year %in% 2015 & Age >= 20), aes(x = Age, y = Ratio, colour = "2015 - Observed")) + 
  ...
  FemalePlot_check = ggplot() + 
    geom_ribbon(data = subset(FLFPRFemale_check, Year %in% 2015 & Age >= 20), aes(x = Age, ymin = Ratio_lo, ymax = Ratio_hi), fill = "grey70", alpha = 0.25) + 
    geom_line(data = subset(FLFPRFemale_check, Year %in% 2015 & Age >= 20), aes(x = Age, y = Ratio, colour = "2015 - Projected")) + 
    geom_line(data = subset(LFPRFemale_interpolation_check, Year %in% 2015 & Age >= 20), aes(x = Age, y = Ratio, colour = "2015 - Observed")) +
    theme_few() + scale_colour_tableau() +
    #scale_colour_manual(name = "Curves", values = cols) + scale_fill_manual(name = "Curves", values = cols) +
    theme(legend.position = "right", legend.title = element_blank())
...
if (check == "N") {
  rm(LFPRMale, LFPRFemale, MatrixMale_interpolation, MatrixFemale_interpolation, StartYear, last_before_proj)
} else {
  rm(LFPRMale, LFPRFemale, MatrixMale_interpolation, MatrixFemale_interpolation,
     ...
     last_before_proj_aux, FLFPRFemale_hi_check, FLFPRFemale_lo_check, FLFPRMale_hi_check, FLFPRMale_lo_check, proj_length_aux)
}
...
FLFPRMale_hi = matrix(nrow = length(MaleModel$kt_hi_forecast), ncol = length(ages))
FLFPRMale_lo = matrix(nrow = length(MaleModel$kt_lo_forecast), ncol = length(ages))

for (i in 1:length(MaleModel$kt_forecast)) {
  FLFPRMale[i, ] <- exp((MaleModel$bx * MaleModel$kt_forecast[i]) + MaleModel$ax)
  FLFPRMale_hi[i, ] <- exp((MaleModel$bx * MaleModel$kt_hi_forecast[i]) + MaleModel$ax)
  FLFPRMale_lo[i, ] <- exp((MaleModel$bx * MaleModel$kt_lo_forecast[i]) + MaleModel$ax)
}

FLFPRMale = t(FLFPRMale)
FLFPRMale_hi = t(FLFPRMale_hi)
FLFPRMale_lo = t(FLFPRMale_lo)

rep.col = function(x,n){
  matrix(rep(x, each = n), ncol = n, byrow = TRUE)
}

constantFLFPRMale = rep.col(FLFPRMale[, ncol(FLFPRMale)], (2100 - tail(years_all, 1)))
constantFLFPRMale_hi = rep.col(FLFPRMale_hi[, ncol(FLFPRMale_hi)], (2100 - tail(years_all, 1)))
constantFLFPRMale_lo = rep.col(FLFPRMale_lo[, ncol(FLFPRMale_lo)], (2100 - tail(years_all, 1)))

if (proj_length == 85){
  FLFPRMale = FLFPRMale
  FLFPRMale_hi = FLFPRMale_hi
  FLFPRMale_lo = FLFPRMale_lo
} else{
  FLFPRMale = cbind(FLFPRMale, constantFLFPRMale)
  FLFPRMale_hi = cbind(FLFPRMale_hi, constantFLFPRMale_hi)
  FLFPRMale_lo = cbind(FLFPRMale_lo, constantFLFPRMale_lo)
}

FLFPRMale = as.data.frame(FLFPRMale)
FLFPRMale_hi = as.data.frame(FLFPRMale_hi)
FLFPRMale_lo = as.data.frame(FLFPRMale_lo)

rownames(FLFPRMale) = ages
rownames(FLFPRMale_hi) = ages
rownames(FLFPRMale_lo) = ages

if (proj_length == 85){
  colnames(FLFPRMale) = years_forecast
  colnames(FLFPRMale_hi) = years_forecast
  colnames(FLFPRMale_lo) = years_forecast
} else{
  colnames(FLFPRMale) = c(years_forecast, seq(tail(years_all, 1) + 1, 2100))
  colnames(FLFPRMale_hi) = c(years_forecast, seq(tail(years_all, 1) + 1, 2100))
  colnames(FLFPRMale_lo) = c(years_forecast, seq(tail(years_all, 1) + 1, 2100))
}
...
rm(Male_kt, Male_kt_forecast, MaleModel, MaleParameters, constantFLFPRMale, constantFLFPRMale_hi, constantFLFPRMale_lo)
...
FLFPRFemale_hi = matrix(nrow = length(FemaleModel$kt_hi_forecast), ncol = length(ages))
FLFPRFemale_lo = matrix(nrow = length(FemaleModel$kt_lo_forecast), ncol = length(ages))

for (i in 1:length(FemaleModel$kt_forecast)) {
  FLFPRFemale[i, ] <- exp((FemaleModel$bx * FemaleModel$kt_forecast[i]) + FemaleModel$ax)
  FLFPRFemale_hi[i, ] <- exp((FemaleModel$bx * FemaleModel$kt_hi_forecast[i]) + FemaleModel$ax)
  FLFPRFemale_lo[i, ] <- exp((FemaleModel$bx * FemaleModel$kt_lo_forecast[i]) + FemaleModel$ax)
}

FLFPRFemale = t(FLFPRFemale)
FLFPRFemale_hi = t(FLFPRFemale_hi)
FLFPRFemale_lo = t(FLFPRFemale_lo)

constantFLFPRFemale = rep.col(FLFPRFemale[, ncol(FLFPRFemale)], (2100 - tail(years_all, 1)))
constantFLFPRFemale_hi = rep.col(FLFPRFemale_hi[, ncol(FLFPRFemale_hi)], (2100 - tail(years_all, 1)))
constantFLFPRFemale_lo = rep.col(FLFPRFemale_lo[, ncol(FLFPRFemale_lo)], (2100 - tail(years_all, 1)))

if (proj_length == 85){
  FLFPRFemale = FLFPRFemale
  FLFPRFemale_hi = FLFPRFemale_hi
  FLFPRFemale_lo = FLFPRFemale_lo
} else{
  FLFPRFemale = cbind(FLFPRFemale, constantFLFPRFemale)
  FLFPRFemale_hi = cbind(FLFPRFemale_hi, constantFLFPRFemale_hi)
  FLFPRFemale_lo = cbind(FLFPRFemale_lo, constantFLFPRFemale_lo)
}

FLFPRFemale = as.data.frame(FLFPRFemale)
FLFPRFemale_hi = as.data.frame(FLFPRFemale_hi)
FLFPRFemale_lo = as.data.frame(FLFPRFemale_lo)

rownames(FLFPRFemale) = ages
rownames(FLFPRFemale_hi) = ages
rownames(FLFPRFemale_lo) = ages

if (proj_length == 85){
  colnames(FLFPRFemale) = years_forecast
  colnames(FLFPRFemale_hi) = years_forecast
  colnames(FLFPRFemale_lo) = years_forecast
} else{
  colnames(FLFPRFemale) = c(years_forecast, seq(tail(years_all, 1) + 1, 2100))
  colnames(FLFPRFemale_hi) = c(years_forecast, seq(tail(years_all, 1) + 1, 2100))
  colnames(FLFPRFemale_lo) = c(years_forecast, seq(tail(years_all, 1) + 1, 2100))
}
...
rm(Female_kt, Female_kt_forecast, FemaleModel, FemaleParameters, ages, i, constantFLFPRFemale, constantFLFPRFemale_hi, 
   constantFLFPRFemale_lo)
...
FLFPRMale_hi = melt(t(FLFPRMale_hi))
FLFPRMale_lo = melt(t(FLFPRMale_lo))

colnames(FLFPRMale) = c("Year", "Age", "Ratio")
colnames(FLFPRMale_hi) = c("Year", "Age", "Ratio")
colnames(FLFPRMale_lo) = c("Year", "Age", "Ratio")
...
FLFPRFemale_hi = melt(t(FLFPRFemale_hi))
FLFPRFemale_lo = melt(t(FLFPRFemale_lo))

colnames(FLFPRFemale) = c("Year", "Age", "Ratio")
colnames(FLFPRFemale_hi) = c("Year", "Age", "Ratio")
colnames(FLFPRFemale_lo) = c("Year", "Age", "Ratio")

ForecastLFPRMale = rbind(LFPRMale_interpolation, FLFPRMale)
ForecastLFPRMale_hi = rbind(LFPRMale_interpolation, FLFPRMale_hi)
ForecastLFPRMale_lo = rbind(LFPRMale_interpolation, FLFPRMale_lo)

ForecastLFPRFemale = rbind(LFPRFemale_interpolation, FLFPRFemale)
ForecastLFPRFemale_hi = rbind(LFPRFemale_interpolation, FLFPRFemale_hi)
ForecastLFPRFemale_lo = rbind(LFPRFemale_interpolation, FLFPRFemale_lo)

rm(FLFPRMale, FLFPRFemale, LFPRMale_interpolation, LFPRFemale_interpolation, FLFPRMale_hi, FLFPRMale_lo, FLFPRFemale_hi, FLFPRFemale_lo)

ggplot(subset(ForecastLFPRMale, Year %in% seq(years_all[2], tail(years_forecast, n = 1), by = 7)), aes(x = Age, y = Ratio, colour = factor(Year))) + 
  geom_line() + theme_few() + scale_colour_tableau() +
  theme(legend.position = "right", legend.title = element_blank()) + 
  geom_line(aes(y = 1), colour = "black", linetype = "dashed") + 
  labs(x = "Ages", y = "Male LFPR - Select Years")

ggplot(subset(ForecastLFPRFemale, Year %in% seq(years_all[2], tail(years_forecast, n = 1), by = 7)), aes(x = Age, y = Ratio, colour = factor(Year))) + 
  geom_line() + theme_few() + scale_colour_tableau() +
...
rm(proj_length, years, years_forecast, years_interpolation, years_all, projStartYear)

#---------------------------------------------------------------------------------------------------------------------
LFPRMale = dcast(ForecastLFPRMale, Age ~ Year, value.var = "Ratio")
LFPRMale_hi = dcast(ForecastLFPRMale_hi, Age ~ Year, value.var = "Ratio")
LFPRMale_lo = dcast(ForecastLFPRMale_lo, Age ~ Year, value.var = "Ratio")

#write.csv(LFPRMale, "TaxasProjMasc_kt.csv")
#write.csv(LFPRMale_hi, "TaxasProjMasc_kt_hi.csv")
#write.csv(LFPRMale_lo, "TaxasProjMasc_kt_lo.csv")

LFPRFemale = dcast(ForecastLFPRFemale, Age ~ Year, value.var = "Ratio")
LFPRFemale_hi = dcast(ForecastLFPRFemale_hi, Age ~ Year, value.var = "Ratio")
LFPRFemale_lo = dcast(ForecastLFPRFemale_lo, Age ~ Year, value.var = "Ratio")

#write.csv(LFPRFemale, "TaxasProjFen_kt.csv")
#write.csv(LFPRFemale_hi, "TaxasProjFem_kt_hi.csv")
#write.csv(LFPRFemale_lo, "TaxasProjFem_kt_lo.csv")
...
#write.csv(resultMale, "ELRPMasc.csv")

resultMale_hi = length.retirement(ForecastLFPRMale_hi, LTMale)
resultMale_hi = melt(resultMale_hi, value.name = "ELRP_high")

#write.csv(resultMale_hi, "ELRPMasc_kt_hi.csv")

resultMale_lo = length.retirement(ForecastLFPRMale_lo, LTMale)
resultMale_lo = melt(resultMale_lo, value.name = "ELRP_low")

#write.csv(resultMale_lo, "ELRPMasc_kt_lo.csv")
...
#write.csv(resultFemale, "ELRPFem.csv")

resultFemale_hi = length.retirement(ForecastLFPRFemale_hi, LTFemale)
resultFemale_hi = melt(resultFemale_hi, value.name = "ELRP_high")

#write.csv(resultFemale_hi, "ELRPFem_kt_hi.csv")

resultFemale_lo = length.retirement(ForecastLFPRFemale_lo, LTFemale)
resultFemale_lo = melt(resultFemale_lo, value.name = "ELRP_low")

#write.csv(resultFemale_lo, "ELRPFem_kt_lo.csv")
...
LifeExpectancyResult = dcast(LifeExpectancy, Year ~ Sex, value.var = "e20")
#write.csv(LifeExpectancyResult, "Expect20M&F.csv")
write.csv(LTMale$qx, "LTMale.csv")
write.csv(LTFemale$qx, "LTFemale.csv")
...
ggplot() + geom_line(data = subset(resultMale, variable %in% seq(1983, 2033)), aes(x = variable, y = ELRP, group = 1, colour = "Base Scenario"), size = 0.65) +
  geom_line(data = subset(ALT1_resultMale, variable %in% seq(1983, 2033)), aes(x = variable, y = ELRP, group = 1, colour = "Constant LFPR"), size = 0.65) +
  geom_line(data = subset(ALT2_resultMale, variable %in% seq(1983, 2033)), aes(x = variable, y = ELRP, group = 1, colour = "Constant Mortality"), size = 0.65) +
  theme_few() + scale_colour_tableau() +
  xlab("Year") + ylab("Male ELRP") +
  scale_x_discrete(breaks = seq(1983, 2033, by = 10)) +
  theme(legend.position = "right", legend.title = element_blank())

ggplot() + geom_line(data = subset(resultFemale, variable %in% seq(1983, 2033)), aes(x = variable, y = ELRP, group = 1, colour = "Base Scenario"), size = 0.65) +
  geom_line(data = subset(ALT1_resultFemale, variable %in% seq(1983, 2033)), aes(x = variable, y = ELRP, group = 1, colour = "Constant LFPR"), size = 0.65) +
  geom_line(data = subset(ALT2_resultFemale, variable %in% seq(1983, 2033)), aes(x = variable, y = ELRP, group = 1, colour = "Constant Mortality"), size = 0.65) +
  theme_few() + scale_colour_tableau() +
